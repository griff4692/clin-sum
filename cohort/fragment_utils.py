...
class Fragments(object):
    ...
    def __init__(self, summary, text, convert_lower=True):
        self._tokens = False
        self.summary = summary
        self.text = text
        self._norm_summary = self._normalize(self.summary, convert_lower)
        self._norm_text = self._normalize(self.text, convert_lower)
        self._match(self._norm_summary, self._norm_text)

    def _normalize(self, tokens, convert_lower=False):
        """
        ...
        return [ str(t).lower() if convert_lower else str(t) for t in tokens]
